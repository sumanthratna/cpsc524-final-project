...
void *Pagerank_Parallel(void *arg) {
  ...
  while (!atomic_load(&global_converged)) {
    ...
    for (i = thread_data->start; i < thread_data->end; i++) {
      ...
      new_p =
          d * (new_p + atomic_load_double(&global_sum)) + (1 - d) * Nodes[i].e;
    ...
    int all_converged = 1;
    for (i = 0; i < num_threads; i++) {
      if (!atomic_load(&Threads_data[i].converged)) {
        all_converged = 0;
        break;
      }
    }
    ...
    double total_sum = 0;
    for (i = 0; i < num_threads; i++) {
      total_sum += atomic_load_double(&Threads_data[i].local_sum);
    }
    ...
    for (i = 0; i < num_threads; i++) {
      double thread_error =
          atomic_load_double(&Threads_data[i].local_max_error);
