...
void Threads_Allocation() {
  ...
  Threads = (pthread_t *)aligned_alloc(CACHE_LINE_SIZE,
                                       num_threads * sizeof(pthread_t));
  Threads_data =
      (Thread *)aligned_alloc(CACHE_LINE_SIZE, num_threads * sizeof(Thread));
...
void Read_from_txt_file(char *filename) {
  ...
  while (fgets(line, sizeof(line), fid) != NULL) {
    ...
    if (sscanf(line, "%d\t%d\n", &from_idx, &to_idx)) {
      ...
      Nodes[to_idx].From_id =
          (int *)realloc(Nodes[to_idx].From_id, temp_size * sizeof(int));
...
void *Pagerank_Parallel(void *arg) {
  ...
  while (!atomic_load(&global_converged)) {
    ...
    for (i = thread_data->start; i < thread_data->end; i += BLOCK_SIZE) {
      int block_end = (i + BLOCK_SIZE < thread_data->end) ? i + BLOCK_SIZE
                                                          : thread_data->end;
      ...
      for (int k = i; k < block_end; k++) {
        ...
        new_p = d * (new_p + atomic_load_double(&global_sum)) +
                (1 - d) * Nodes[k].e;
    ...
    for (i = 0; i < num_threads; i++) {
      double thread_error =
          atomic_load_double(&Threads_data[i].local.local_max_error);
