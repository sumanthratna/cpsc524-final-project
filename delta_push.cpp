...
parlay::sequence<double> pagerank_delta_push(const Graph &G, double epsilon,
  ...
  parlay::parallel_for(0, n, [&](size_t i) { deltas[i] = ranks[i]; });
  ...
  while (max_delta > epsilon && iter < max_iters) {
    ...
    parlay::parallel_for(0, n, [&](size_t i) {
      if (std::fabs(deltas[i]) >
          epsilon * 0.01) { // Using a threshold for active vertices
        ...
        if (G.out_degrees[i] > 0) {
          ...
          for (size_t j = G.out_offsets[i]; j < G.out_offsets[i + 1]; ++j) {
            ...
            __atomic_add_fetch((long *)&next_deltas[neighbor],
                               *(long *)&delta_per_edge, __ATOMIC_RELAXED);
          }
        }
    ...
    auto delta_abs = parlay::tabulate<double>(
        n, [&](size_t i) { return std::fabs(next_deltas[i]); });
    ...
    iter++;
    if (iter % 10 == 0) {
      std::cout << "Iteration " << iter << ", max_delta = " << max_delta
                << std::endl;
    }
  }

  std::cout << "Converged in " << iter
            << " iterations; final error = " << max_delta << "\n";
  ...
  // Normalize so sum = 1
  parlay::parallel_for(0, n, [&](size_t i) { ranks[i] /= sum; });
...
int main(int argc, char **argv) {
  if (argc < 2) {
    std::cerr << "Usage: " << argv[0]
              << " <edge_list.txt> [num_vertices] [epsilon] [damping] "
                 "[max_iters] [algorithm]\n";
  ...
  std::ofstream output_file("delta_push.txt", std::ios::app);
